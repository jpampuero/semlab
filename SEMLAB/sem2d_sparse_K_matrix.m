...
W     = zeros(NGLL,NGLL);	% for internal forces % for homogeneous medium
...
%  W(:,:,e) = wgll2 .* mu; 
% we'll define this outside the loop, assuming homogeneous medium (same W for all elements)
...
W = WMatrix(NELX, NELY, NGLL, wgll2, dxe, dye, RHO, VS);

% assemble stiffness as a sparse matrix
Ksparse = assemble_K_matrix_2d(NELX, NELY, NGLL, dxe, dye, nglob, iglob,W);
...
% STEP 4: OUTPUT
  ...
  cpu_e = cputime-cpu_t;
if(it<20),  disp(['Estimated time is ', num2str(cpu_e*NT/60),' min ',num2str(cpu_e)]); end
end % ... of time loop
